...
if (isset($_GET['asset_id'])) {
    ...
    if (empty($asset_ip)) {
        ...
        $asset_ip_display = "$asset_ip<button class='btn btn-sm'
            data-clipboard-text=" . $asset_ip . "><i class='far fa-copy text-secondary'></i></button>";
    }
    ...
    <div class="row">
        ...
        <div class="col-md-3">
            ...
            <div class="card card-dark">
                <div class="card-body">
                    <h3 class="text-bold">
                        <i class="fa fa-fw text-secondary fa-<?php echo $device_icon; ?> mr-3"></i>
                        <?php echo $asset_name; ?>
                    </h3>
                    ...
                    <?php if (!empty($location_name)) { ?>
                        <div class="mb-1">
                            <i class="fa fa-fw fa-map-marker-alt text-secondary mr-3"></i>
                            <?php echo $location_name_display; ?>
                        </div>
                    <?php }
                    if (!empty($contact_email)) { ?>
                        <div>
                            <i class="fa fa-fw fa-envelope text-secondary mr-3"></i>
                    ...
                    if (!empty($contact_phone)) { ?>
                        <div class="mb-2">
                            <i class="fa fa-fw fa-phone text-secondary mr-3"></i>
                            <?php echo "$contact_phone $contact_extension"; ?>
                        </div>
                    <?php }
                    if (!empty($contact_mobile)) { ?>
                        <div class="mb-2">
                            <i class="fa fa-fw fa-mobile-alt text-secondary mr-3"></i>
                            <?php echo $contact_mobile; ?>
                        </div>
                    <?php }
                    if (!empty($contact_pin)) { ?>
                        <div class="mb-2">
                            <i class="fa fa-fw fa-key text-secondary mr-3"></i>
                            <?php echo $contact_pin; ?>
                        </div>
                    <?php } ?>
                    <div class="mb-2">
                        <i class="fa fa-fw fa-clock text-secondary mr-3"></i>
                        <?php echo date('Y-m-d', strtotime($asset_created_at)); ?>
                    </div>
                    <hr>
                    <button type="button" class="btn btn-primary"
                        data-toggle="modal" data-target="#editAssetModal<?php echo $asset_id; ?>">
 ...
 ?>
            ...
            <div class="card mb-3">
                ...
                <div class="card-body p-1">
                    <textarea class="form-control" rows=6 id="assetNotes"
                        placeholder="Enter quick notes here" onblur="updateAssetNotes(<?php echo $asset_id ?>)">
                        <?php echo $asset_notes ?>
                    </textarea>
                </div>
            </div>
                        ...
                        <table class="table table-striped table-borderless table-hover">
                            ...
                            while ($row = mysqli_fetch_array($sql_related_logins)) {
                                ...
                                if (empty($login_uri)) {
                                    ...
                                    $login_uri_display = "$login_uri<button class='btn btn-sm clipboardjs'
                                    data-clipboard-text='$login_uri'><i class='far fa-copy
                                    text-secondary'></i></button><a href='https://$login_uri'
                                    target='_blank'><i class='fa fa-external-link-alt text-secondary'></i></a>
                                    ";
                                }
                                $login_username = nullable_htmlentities(decryptLoginEntry($row['login_username']));
                                if (empty($login_username)) {
                                    ...
                                    $login_username_display = "$login_username<button class='btn btn-sm clipboardjs'
                                    data-clipboard-text='$login_username'><i class='far fa-copy text-secondary'>
                                    </i></button>
                                    ";
                                }
                                $login_password = nullable_htmlentities(decryptLoginEntry($row['login_password']));
                                ...
                                if (empty($login_otp_secret)) {
                                    ...
                                    $otp_display = "<span onmouseenter='showOTP($login_id_with_secret)'>
                                        ...
                                        <span id='otp_$login_id'><i>Hover..</i>
                                    </span></span>";
                                }
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a tabindex="0" href="#" data-toggle="popover"
                                            data-trigger="focus" data-placement="top"
                                            data-content="<?php echo $login_password; ?>">
                        ...
                        <table class="table table-striped table-borderless table-hover">
                            ...
                            while ($row = mysqli_fetch_array($sql_related_software)) {
                                ...
                                $contact_licenses_sql = mysqli_query(
                                    $mysqli, "SELECT contact_id FROM software_contacts
                                    WHERE software_id = $software_id");
